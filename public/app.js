...
async function activity(){
 ...
 const person=(id,username)=>{const name=username||identities.get(id);const box=el('div',null,'activity-person');box.append(el('span',name?name[0].toUpperCase():'◉','activity-avatar'),el('strong',name?'@'+name:(id?'Perfil · '+id:'Perfil não informado')));return box;};
 ...
 for(const e of incoming){const card=el('article',null,'interaction-card'),top=el('div',null,'interaction-top');top.append(el('span',labels[e.kind],'activity-tag event-'+e.kind),person(e.info.userId,e.info.username),el('time',date(e.created),'activity-date'));card.append(top);if(e.info.text)card.append(el('blockquote',e.info.text));const details=el('details');details.append(el('summary','Ver detalhes técnicos'),el('p','Identificador do evento: '+e.info.eventId));card.append(details);host.append(card);}
