...
export type Flow={version:1;map?:MapFlow;match:'contains'|'exact';allPosts:boolean;welcomeEnabled:boolean;welcomeText:string;welcomeButton:string;requireFollow:boolean;followText:string;collectEmail:boolean;emailText:string;linkEnabled:boolean;linkLabel:string;attachmentType:''|'image'|'audio'|'video';attachmentUrl:string;followupEnabled:boolean;followupMinutes:number;followupText:string;followupLink:string;followupLabel:string};
...
export function validateFlow(input:unknown):Flow{
 ...
 const match=str('match','contains',10);if(!['contains','exact'].includes(match))throw Error('Correspondência inválida.');
 const attachmentType=str('attachmentType','',10);if(!['','image','audio','video'].includes(attachmentType))throw Error('Tipo de mídia inválido.');
